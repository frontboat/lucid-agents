...
describe("create-agent-kit CLI", () => {
  ...
  it("accepts template arguments via CLI flags in non-interactive mode", async () => {
    ...
    expect(envFile).toContain("PAYMENTS_RECEIVABLE_ADDRESS=0x1234567890123456789012345678901234567890");
  ...
  it("CLI arguments with special characters are handled correctly", async () => {
    ...
    expect(envFile).toContain("AGENT_DESCRIPTION=Agent with special chars: @#$%&");
    expect(envFile).toContain("PAYMENTS_FACILITATOR_URL=https://facilitator.example.com/api?key=test");
  });
  ...
  it("AGENTS.md and template.schema.json are copied to generated project", async () => {
    ...
    const templateSchema = await readFile(join(projectDir, "template.schema.json"), "utf8");
